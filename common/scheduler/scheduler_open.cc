...
#include "policies/reliabilityAware.h"
...
void SchedulerOpen::initMappingPolicy(String policyName) {
	...
	} else if (policyName == "reliabilityAware")  { 
		cout << "[Scheduler] [Info]: Reliability-aware mapping policy" << endl;
		float rValueThreshold = Sim()->getCfg()->getFloat( "scheduler/open/migration/reliabilityAware/rValueThreshold");
		mappingPolicy = new ReliabilityAware(performanceCounters, coreRows, coreColumns, rValueThreshold);
	} else {
...
void SchedulerOpen::initMigrationPolicy(String policyName) {
	...
	if (policyName == "off") {
		...
		cout << "[Scheduler] [Info]: Reliability-aware migration policy" << endl;
		float rValueThreshold = Sim()->getCfg()->getFloat( "scheduler/open/migration/reliabilityAware/rValueThreshold");
		migrationPolicy = new ReliabilityAware(performanceCounters, coreRows, coreColumns, rValueThreshold);
	} else {
