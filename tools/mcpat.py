...
from config import SNIPER
sys.path.append(SNIPER)

from pggen import pg_gen
from proton import PROTON_analyze
...
def power_stack(power_dat, cfg, seconds, powertype='total', nocollapse=False):
    ...
    if (sniper_config.get_config(cfg, "periodic_thermal/enabled") == 'true'):
        ...
        # --- New Section: Read latest log lines and process them ---
        logger.debug("Starting power-grid generation")
        output_dir = sniper_config.get_config(cfg, "general/output_dir")
        logger.debug("HotPROTON output directory: %s", output_dir)
        
        
        # Read header and latest reading from PeriodicPower.log
        power_log_path = os.path.join(output_dir, "PeriodicPower.log")
        with open(power_log_path, 'r') as pf:
            power_lines = pf.readlines()
        power_file_length = len(power_lines)  # number of lines in the file
        if len(power_lines) >= 2:
            power_header = power_lines[0].strip()
            power_reading = power_lines[-1].strip()
        else:
            power_header, power_reading = "", ""
        
        # Read header and latest reading from PeriodicThermal.log, if thermal logging is enabled
        if sniper_config.get_config(cfg, "periodic_thermal/enabled") == 'true':
            thermal_log_path = os.path.join(output_dir, "PeriodicThermal.log")
            with open(thermal_log_path, 'r') as tf:
                thermal_lines = tf.readlines()
            if len(thermal_lines) >= 2:
                thermal_header = thermal_lines[0].strip()
                thermal_reading = thermal_lines[-1].strip()
            else:
                thermal_header, thermal_reading = "", ""
        else:
            thermal_header = thermal_reading = ""

        # Read header and latest reading from PeriodicVdd.log
        vdd_log_path = os.path.join(output_dir, "PeriodicVdd.log")
        with open(vdd_log_path, 'r') as vf:
            vdd_lines = vf.readlines()
        vdd_file_length = len(vdd_lines)
        if len(vdd_lines) >= 2:
            vdd_header = vdd_lines[0].strip().split()  # ['Core0', 'Core1', 'Core2', 'Core3']
            vdd_reading = vdd_lines[-1].strip().split()  # ['0.800', '0.800', '0.800', '0.800']
            # Map header to values and convert to float, rename keys from CoreX to C_X
            voltage_data = {
                "C_{}".format(col[4:]): float(val)
                for col, val in zip(vdd_header, vdd_reading)
            }
        else:
            voltage_data = {}

    # Process the log lines to get dictionaries with power and thermal data
    power_data, thermal_data = process_log_lines(power_header, power_reading, thermal_header, thermal_reading)
    calculate_physics_based_reliability_metrics(power_data, thermal_data, voltage_data, output_dir, int((power_file_length - 1) * 1000* seconds))


    return buildstack.merge_items({0: data}, all_items, nocollapse=nocollapse)

################### POWERGRID GENERATOR FUNCTIONS ###################
def calculate_physics_based_reliability_metrics(power_data, thermal_data, voltage_data, output_dir, seconds):
    # Generate the power grid
    powergrids_dir = os.path.join(output_dir, "powergrids/")
    if not os.path.exists(powergrids_dir):
        os.makedirs(powergrids_dir)
        
    pg_gen.pg_gen(power_data, thermal_data, voltage_data, powergrids_dir, seconds)

    # Find all spice files in the output directory
    spice_files = glob.glob(os.path.join(powergrids_dir, "C_*_T{}_b.spice".format(seconds)))
    spice_files.sort()  # Explicit sorting for consistency
    logger.debug("Generated SPICE files: %s", spice_files)
    # Loop through each spice file and run PROTON_analyze
    for spice_file in spice_files:
        # Extract core name (e.g., C_0 from C_0_T0_b.spice)
        core_name = os.path.basename(spice_file).split("_T")[0]
        logger.debug("Processing SPICE file: %s", spice_file)
        # Run PROTON_analyze with the extracted spice file and core name
        PROTON_analyze.PROTON_analyze(spice_file, core_name, seconds)
        logger.debug("Finished processing SPICE file: %s", spice_file)
    
    create_instantaneous_rvalue_log(output_dir)
        

def process_log_lines(power_header, power_reading, thermal_header, thermal_reading):
    """
    Process the header and reading lines from the power and thermal logs.
    Splits the headers and values and converts the values to floats.
    Returns dictionaries mapping each component to its value.
    """
    # Split the headers into component names
    power_components = power_header.split('\t')
    thermal_components = thermal_header.split('\t')
    
    # Split the reading lines into values
    power_values = power_reading.split('\t')
    thermal_values = thermal_reading.split('\t')
    
    # Convert string values to floats, handling missing values if needed.
    try:
        power_values = [float(val) for val in power_values if val != '']
    except ValueError:
        power_values = [None for val in power_values]
    
    try:
        thermal_values = [float(val) for val in thermal_values if val != '']
    except ValueError:
        thermal_values = [None for val in thermal_values]
    
    # Build dictionaries mapping component names to their values.
    power_data = dict(zip(power_components, power_values))
    thermal_data = dict(zip(thermal_components, thermal_values))
    
    logger.debug("Latest power data: %s", power_data)
    logger.debug("Latest thermal data: %s", thermal_data)
    
    return power_data, thermal_data

def create_instantaneous_rvalue_log(output_dir):
    """
    Create or overwrite InstantaneousRvalue.log with:
    - First line: Subcore names from all PeriodicRvalues_<core_name> files.
    - Second line: Latest R(t) values from all PeriodicRvalues_<core_name> files.
    """
    core_files = [os.path.join(output_dir, "PeriodicRvalues_C_{}".format(i)) for i in range(4)]
    log_file_path = os.path.join(output_dir, "InstantaneousRvalue.log")
    ...
    try:
        for file_path in core_files:
            if not os.path.exists(file_path):
                raise IOError("Missing file: {}".format(file_path))

            with open(file_path, "r") as f:
                lines = f.readlines()
                if len(lines) < 2:
                    raise ValueError("File {} does not have enough data.".format(file_path))

                subcore_names.append(lines[0].strip())  # First line (subcore names)
                last_r_values.append(lines[-1].strip())  # Last line (latest R values)

        # Write to InstantaneousRvalue.log (overwrite mode)
        with open(log_file_path, "w") as log_file:
            log_file.write(" ".join(subcore_names) + "\n")  # Concatenate subcore names
            log_file.write(" ".join(last_r_values) + "\n")  # Concatenate latest R values

    except Exception as e:
        print("Error creating InstantaneousRvalue.log: {}".format(e))


############## POWERGRID GENERATOR FUNCTIONS END HERE ##############
...
def update_reliability_values(cfg, instant_temperatures, delta_t_s):
    ...
    logger.debug("Executing reliability command: %s", reliability_cmd)
