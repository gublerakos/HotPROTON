...
class Matrix_Formulation_Class(object):  # Explicitly inherit from `object` for compatibility with Python 2 and 3
  def __init__(self, csv_file, project_location, sp_step, given_disc_point, technology, temperature, givenWidth):
      super(Matrix_Formulation_Class, self).__init__()  # Explicitly pass the class and `self` for Python 2
  ...
  def matrix_formulation(self):
    ...
    logger.debug("Formulating matrices for %s", self.csv_file)
    ...
    try:
      with open(csv_filename) as f:
        ...
        if(self.given_disc_point!=None):
          ...
          logger.debug("Requested discretization points: %s", self.given_disc_point)
    ...
    matrix_G = [[0.0 for x in range(int(nx_total))] for y in range(int(nx_total))]
    matrix_B = [[0.0 for x in range(int(num_segments))] for y in range(int(nx_total))]
    matrix_L = [[0.0 for x in range(int(nx_total))] for y in range(int(num_segments+1))]
    curden = [0.0 for x in range(int(num_segments))]
    monitor_points = [0.0 for x in range(int(num_segments+1))]
    ...
    ######################################
    #Create matrix G
    help_matrix = np.ones((int(nx_total), int(nx_total)), float) #an array with ones
    ...
    matrix_G[0][0] = -1.0
    matrix_G[int(nx_total-1)][int(nx_total-1)] = -1.0
    ...
    monitor_points[0] = 1
    i = 1
    logger.debug("Via-to-segment map: %s", vias)
    for node in vias_nodes:                       #via_nodes has the via nodes that are key to the dictionary vias
      segment = vias[node]
      logger.debug("Via node %s belongs to segment %s", node, segment)
      matrix_B[int(node-1)][int(segment-2)] = -1.0     #each via node is at the second segment that why we decrease 2 einai mia thesi pio mesa apo tin teleytaia stili poy einai segents-1 ara segments-2
      matrix_B[int(node-1)][int(segment-1)] = 1.0
      matrix_L[int(segment-1)][int(node-1)] = 1.0
      monitor_points[int(segment-1)] = node
      i += 1
    matrix_B[int(nx_total-1)][int(num_segments-1)] = -1.0  #the last line of the array B
    matrix_L[int(num_segments)][int(nx_total-1)] = 1.0
    monitor_points[int(num_segments)] = nx_total

    logger.debug("Via nodes: %s", vias_nodes)
    ########
    acoef = kappa/(dx*dx)
    bcoef = kappa * b / dx
    ...
    logger.debug("Formulated %s nodes in %.3f seconds", nx_total, elapsed_time)
    return("%d nodes in %.3f seconds" % (nx_total, elapsed_time))
