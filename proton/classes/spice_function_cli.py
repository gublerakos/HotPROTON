...
def change_directory_filename(filename, project_location):
    ...
    new_filename = os.path.join(project_location, "{}{}".format(root_name, extension))
    return new_filename

def remove_strings_with_substring(strings, substring):
    return [string for string in strings if substring not in string]

class Spice_Parser_Class(object):  # Explicitly inherit from `object` for compatibility with Python 2 and 3
  def __init__(self, spice_file, project_location, IS_LINUX, INSTALLATION_FOLDER):
      super(Spice_Parser_Class, self).__init__()  # Explicitly pass the class and `self` for Python 2
  ...
  def spice_parser(self): 
    ...
    logger.debug("Parsing SPICE file: %s", spice_file)
    benchmark= project_location+"/"+foldername
    # removing this in order not to run circuit simulation, because it is already done
    # adding else with the testcases - PROBLEM WITH CBLAS library while running kalonakis
    if IS_LINUX:
      # Remove .op if exists
      ...
      process = subprocess.Popen([exec_path, spice_file], stdout=subprocess.PIPE, stderr=subprocess.PIPE)
      stdout, stderr = process.communicate()  # Get output and errors
      if process.returncode == 0:  # Command succeeded
        logger.debug("Circuit simulation output: %s", stdout.decode('utf-8'))
      else:  # Command failed
        print("Error: " + stderr.decode('utf-8'))  # Decode bytes to string
        ...
        for line in lines:
          ...
          if not found_layers:
            ...
            if line_components[0][0] == 'R':
              ...
              folder = benchmark + "/"
              logger.debug("Ensuring benchmark directory exists: %s", folder)
          ...
          if line[0] == 'R':
            ...
            if line_words[0] not in resistors:
              ...
              return "{} has already been parsed.".format(line_words[0])
            ...
            if left_coords[0] != right_coords[0]:  # via
              logger.debug("Skipping nodes on different nets: %s", line.rstrip())
              continue
            ...
            if segment_number == 0:
              ...
              logger.debug("Writing parsed line to %s", filename)
              f = open(filename, "w")
            ...
            if length > 0:
              ...
              segment_current = 0
              logger.debug("Skipping zero-length segment %s: %s", line_number, line.rstrip())
              out = True
              continue
          ...
          # If comment *layer is found , keep layer and node in a dictionary
            ...
            if 'n'+str(line_components[4]) not in layers_dic:
              ...
              current_net = "%s_n%s" % (layers_dic["n" + str(line_components[4])], line_components[4])
              logger.debug(
                "Found layer %s, net %s",
                layers_dic["n" + str(line_components[4])],
                line_components[4],
              )
    ...
    logger.debug("Finished parsing %s in %.3f seconds", spice_file, elapsed_time)
    # return (0,"Successfully parsed spice file.")
    return "in %.3f seconds" % elapsed_time
