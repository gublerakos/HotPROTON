...
void FrontendControl <T>::openFile(threadid_t threadid)
{
  ...
  try {
    ...
    // The private remote-trace transport from the development tree is not
    // portable. Fail explicitly if that unsupported option is requested.
    if (m_options->get_ssh())
    {
      std::cerr << "The remote SIFT transport is not supported by HotPROTON." << std::endl;
      exit(1);
    }
  } catch (...) {
